...
void hormone() {
  ...
  if (count >= 10) {
    count = 0;
    *ptr_sum_hi = ((hi[0] + hi[1] + hi[2] + hi[3] + hi[4] + hi[5] + hi[6] + hi[7] + hi[8] + hi[9]) / (10 * 200));
    *ptr_hg = sigmoid(*ptr_sum_hi / 1.0);
    *ptr_hc = *ptr_beta * *ptr_hc + *ptr_alphah * *ptr_hg;
    *ptr_mi = (*ptr_gramma * *ptr_hc) + 0.09;
    if (*ptr_mi < 0.05) {
      *ptr_mi = 0.05;
    } else if (*ptr_mi > 0.09) {
      *ptr_mi = 0.09;
    }
    if (tired == false && *ptr_mi <= 0.06) {
      tired = true;
    } else if (tired == true && *ptr_mi >= 0.08) {
      tired = false;
    }
    // mi
    *ptr_wso0so1 = (*ptr_alpha * sin(*ptr_phi + *ptr_mi));
    *ptr_wso1so0 = (*ptr_alpha * -sin(*ptr_phi + *ptr_mi));
    // Serial.println(String(*ptr_beta) + " " + String(*ptr_alphah) + " " + String(*ptr_mi) + " " + String(*ptr_sum_hi) + " " + String(*ptr_hg) + " " + String(*ptr_so0o));
  } else {
    hi[count] = (abs(sc.ReadLoad(motor1)) + abs(sc.ReadLoad(motor2)) + abs(sc.ReadLoad(motor3)) + abs(sc.ReadLoad(motor4)) + abs(sc.ReadLoad(motor5)) + abs(sc.ReadLoad(motor6)) + abs(sc.ReadLoad(motor7)) + abs(sc.ReadLoad(motor8)) + abs(sc.ReadLoad(motor9)) + abs(sc.ReadLoad(motor11)) + abs(sc.ReadLoad(motor12))) / 11;
    // hi[count] = sc.ReadLoad(1) + sc.ReadLoad(2) + sc.ReadLoad(3) + sc.ReadLoad(4) + sc.ReadLoad(5) + sc.ReadLoad(6) + sc.ReadLoad(7) + abs(sc.ReadLoad(8)) + sc.ReadLoad(9) + sc.ReadLoad(11) + sc.ReadLoad(12);
    // hi[count] = abs(sc.ReadLoad(1));
    count += 1;
  }
}
...
void leg_cpg() {
  ...
  *ptr_vrn14c = (*ptr_wvrn10vrn14 * *ptr_vrn10o) + (*ptr_wvrn11vrn14 * *ptr_vrn11o) + (*ptr_wvrn12vrn14 * *ptr_vrn12o) + (*ptr_wvrn13vrn14 * *ptr_vrn13o);
  ...
  //   *ptr_so0c = 0.0;
  //   *ptr_so1c = 0.0;
  //   // Serial.println(String(*ptr_so0c) + " " + String(*ptr_so1c) + " true");
  // } else if (tired == false && (*ptr_so0c == 0.0 || *ptr_so1c == 0.0)) {
  //   *ptr_so0c = 0.01;
  //   *ptr_so1c = 0.01;
  //   // Serial.println(String(*ptr_so0c) + " " + String(*ptr_so1c) + " false");
  // }
...
void onMqttMessage(int messageSize) {
  ...
  while (mqttClient.available()) {
    ...
    if (error) {
      ...
      if (doc["range"][0] && doc["range"][1] && doc["range"][2] && doc["range"][3]) {
        range[0] = doc["range"][0];
        range[1] = doc["range"][1];
        range[2] = doc["range"][2];
        range[3] = doc["range"][3];
  ...
  *ptr_currentMillis = millis();
  if (*ptr_currentMillis - *ptr_previousMillis >= *ptr_interval) {
    ...
    *ptr_previousMillis = *ptr_currentMillis;
    ...
    if (mode == 'g') {
      // leg
      sc.WritePos(motor4, my_map(*ptr_m4o, -0.94, 0.94, 380, 480), *ptr_interval);
      sc.WritePos(motor5, my_map(*ptr_m5o, -0.94, 0.94, 300, 500), *ptr_interval);
      sc.WritePos(motor9, my_map(*ptr_m9o, -0.94, 0.94, 490, 590), *ptr_interval);
      sc.WritePos(motor1, my_map(*ptr_m1o, -0.94, 0.94, 500, 700), *ptr_interval);
      // sc.WritePos(4, 500, *ptr_interval);
      // sc.WritePos(5, 500, *ptr_interval);
      // sc.WritePos(9, 500, *ptr_interval);
      // sc.WritePos(3, 500, *ptr_interval);
      // hand
      sc.WritePos(motor6, my_map(*ptr_m6o, -0.94, 0.94, 0, 200), *ptr_interval);
      sc.WritePos(motor12, my_map(*ptr_m12o, -0.94, 0.94, 800, 1000), *ptr_interval);
      // body & tail
      sc.WritePos(motor11, my_map(*ptr_m11o, -0.94, 0.94, 425, 575), *ptr_interval);
      sc.WritePos(motor2, my_map(*ptr_m2o, -0.94, 0.94, 525, 675), *ptr_interval);
      sc.WritePos(motor3, my_map(*ptr_m3o, -0.94, 0.94, 425, 575), *ptr_interval);
      sc.WritePos(motor8, my_map(*ptr_m8o, -0.94, 0.94, 425, 575), *ptr_interval);
      sc.WritePos(motor7, my_map(*ptr_m7o, -0.94, 0.94, 575, 725), *ptr_interval);
    } else if (mode == 's' && tired == false) {
      // swim mode
      sc.WritePos(motor4, 800, *ptr_interval);
      sc.WritePos(motor5, 900, *ptr_interval);
      sc.WritePos(motor9, 200, *ptr_interval);
      sc.WritePos(motor1, 125, *ptr_interval);
      sc.WritePos(motor6, 20, *ptr_interval);
      sc.WritePos(motor12, 1000, *ptr_interval);
      sc.WritePos(motor11, my_map(*ptr_m11o, -0.94, 0.94, 425, 575), *ptr_interval);
      sc.WritePos(motor2, my_map(*ptr_m2o, -0.94, 0.94, 525, 675), *ptr_interval);
      sc.WritePos(motor3, my_map(*ptr_m3o, -0.94, 0.94, 425, 575), *ptr_interval);
      sc.WritePos(motor8, my_map(*ptr_m8o, -0.94, 0.94, 425, 575), *ptr_interval);
      ...
      sc.WritePos(motor4, 500, *ptr_interval);
      sc.WritePos(motor5, 500, *ptr_interval);
      sc.WritePos(motor9, 500, *ptr_interval);
      sc.WritePos(motor3, 500, *ptr_interval);

      sc.WritePos(motor11, 500, *ptr_interval);
      sc.WritePos(motor2, 600, *ptr_interval);
      sc.WritePos(motor1, 500, *ptr_interval);
      sc.WritePos(motor8, 500, *ptr_interval);
      sc.WritePos(motor7, 650, *ptr_interval);

      sc.WritePos(motor6, 0, *ptr_interval);
      sc.WritePos(motor12, 1000, *ptr_interval);
      // mode = 't';
    } else if (mode == 'd' && tired == false) {
      // leg
      for (unsigned char cID = 0; cID <= 3; cID++) {
        static int idnum[] = { motor9, motor1, motor4, motor5 };
      ...
      Serial.println(String(sc.ReadLoad(motor1)) + " " + String(sc.ReadLoad(motor2)) + " " + String(sc.ReadLoad(motor3)) + " " + String(sc.ReadLoad(motor4)) + " " + String(sc.ReadLoad(motor5)) + " " + String(sc.ReadLoad(motor6)) + " " + String(sc.ReadLoad(motor7)) + " " + String(sc.ReadLoad(motor8)) + " " + String(sc.ReadLoad(motor9)) + " " + String(sc.ReadLoad(motor11)) + " " + String(sc.ReadLoad(motor12)));
    } else {
      sc.EnableTorque(0xfe, 0);
    }
    ...
    Serial.println(String(*ptr_beta) + " " + String(*ptr_alphah) + " " + String(*ptr_mi) + " " + String(*ptr_hc) + " " + String(*ptr_so0o) + " " + String(*ptr_so1o) + " " + String(*ptr_hg) + " " + String(tired));
